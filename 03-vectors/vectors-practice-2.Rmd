...
```{r}
by.best.mpg <- mtcars[order(mtcars$mpg, -mtcars$hp),]
```
...
```{r}
new.p1x <- p1x[!(p1y > 75 & p1y < 100)]
new.p1y <- p1y[!(p1y > 75 & p1y < 100)]
...
```{r}
new.hobbes.x <- p2x[p2y>50] + 50
new.hobbes.y <- p2y[p2y>50]
draw(c(p2x, new.hobbes.x), c(p2y, new.hobbes.y))
```
...
# 42 and 45 chosen by trail and error
new.p3x[p3x < 42 & p3y < 45] <- p3y[p3x < 42 & p3y < 45]
new.p3y[p3x < 42 & p3y < 45] <- p3x[p3x < 42 & p3y < 45]
